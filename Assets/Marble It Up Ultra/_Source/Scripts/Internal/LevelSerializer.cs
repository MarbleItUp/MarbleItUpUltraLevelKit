...
public class LevelSerializer
{
    ...
    void SerializeLightmaps(ref SerializerHelper sh)
    {
        ...
        {
            failCause = "Lightmap Color Texture must be DXT5 Crunched. Check requirements here: https://github.com/MarbleItUp/MarbleItUpUltraLevelKit/wiki/Lightmap-Settings";
            Selection.activeObject = color;
        }

        if (mask != null && mask.format != TextureFormat.DXT1Crunched)
        {
            failCause = "Lightmap Shadow Texture must be DXT1 Crunched. Check requirements here: https://github.com/MarbleItUp/MarbleItUpUltraLevelKit/wiki/Lightmap-Settings";
    ...
    bool SerializeParticleProxy(GameObject go, LevelObject lo)
    {
        var proxy = go.GetComponent<CloudProxy>();
        if (!proxy)
            return false;
        
        var ps = go.GetComponent<ParticleSystem>();
        ps.Play();
        ps.Simulate(1.0f, true, true, false);

        ParticleSystem.Particle[] particles = new ParticleSystem.Particle[128];
        var particleCount = ps.GetParticles(particles);
        for (var i = 0; i < particleCount; i++)
        {
            var p = particles[i];

            var childLo = new LevelObject();
            childLo.properties[LevelObject.PARTICLE_SYSTEM_PROXY] = 1;
            childLo.properties[LevelObject.PARTICLE_SYSTEM_PROXY_ID] = proxy.ParticleSystemProxyId;

            childLo.position = p.position;
            childLo.scale = p.GetCurrentSize3D(ps);
            childLo.rotation = Quaternion.AngleAxis(p.rotation, p.axisOfRotation);

            lo.children.Add(childLo);
        }
    ...
    LevelObject SerializeGameObject(GameObject go, bool rejectStatic = true)
    {
        ...
        if (SerializeParticleProxy(go, lo))
            return lo;
    ...
    bool SerializeKnownComponents(GameObject go, LevelObject lo)
    {
        ...
        if (ps != null && ps.main.maxParticles != 0)
        {
            ...
            // MARBLR: hack for shooting stars -- strip the sub-emitter off this particle system...
            ParticleSystem sub = null;
            if (ps.subEmitters.subEmittersCount > 0)
            {
                sub = ps.subEmitters.GetSubEmitterSystem(0);
            ...
            // ...then add it back after serializing.
            if (sub != null)
                ps.subEmitters.AddSubEmitter(sub, ParticleSystemSubEmitterType.Birth, ParticleSystemSubEmitterProperties.InheritNothing);
    ...
    {
        var gt = go.GetComponent<GemTrigger>();
        if(gt != null)
        {
            lo.properties[LevelObject.GEMTRIGGER] = true;
            lo.properties[LevelObject.GEMTRIGGER_COMPARISONMODE] = (int)gt.ComparisonMode;
        ...
        {
            lo.properties[LevelObject.CHECKPOINT] = true;
            lo.properties[LevelObject.CHECKPOINT_ORDER] = (int)checkpoint.Order;
        }
    }
    ...
    bool SerializePrefab(GameObject go, LevelObject lo)
    {
        ...
        var fixedName = GetPrefabID(go.name, null);
